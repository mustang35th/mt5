...
H1推移は、H1新規足ごとに保存した観測を時系列で表示します。1行に取得時スプレッドと、MN1・W1・D1・H4・H1の分析方向、Elliott、波動状態、EMA200、GMMA、Stochastic、ATRをまとめます。Observation表がまだ作成されていない場合もアラート一覧は通常どおり利用でき、H1推移側だけが未利用表示になります。スプレッド追加前の過去行は「未記録」と表示します。
