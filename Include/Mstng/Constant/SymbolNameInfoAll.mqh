 ...
 * 通貨ペア情報の一覧を管理するクラス。
 *
 * - 主要通貨ペア（JPYクロス, USDクロス など）を内部リストに保持
 * - シンボル文字列からコード・GMOフラグを取得
 * - コードからシンボル文字列を取得
 */
...
public:
     ...
     * デストラクタ。
     *
     * 内部で保持している SymbolNameInfo インスタンスを解放する。
     */
    ~SymbolNameInfoAll() {
     ...
     *
     * @param index インデックス
     * @return      SymbolNameInfo へのポインタ（存在しない場合は NULL）
     */
