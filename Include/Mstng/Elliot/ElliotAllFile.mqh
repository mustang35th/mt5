...
public:
    ...
    void setupMultiTimeFrameSameFolder(
        ...
        MarketContext context(symbolNameValue, startTimeFrameValue);
        this.initializeMarketContext(context);
        this.folderName = folderNameValue;
    ...
    /**
     * 市場コンテキストを使用して同一フォルダ内の複数時間足CSVを設定する。
     *
     * @param folderNameValue フォルダ名
    ...
    string getSymbolName() const {
        return this.marketContext.symbolName;
    }
    ...
    ENUM_TIMEFRAMES getTimeFrame() const {
        return this.marketContext.timeFrame;
    }
    ...
    string getTimeFrameLabel() const {
        return this.marketContext.timeFrameLabel;
    }
     ...
     */
    string getKey() const {
        return this.marketContext.symbolName + "_" + this.marketContext.timeFrameLabel;
    }
...
private:
    ...
    void createCommonValues(string &commonValues[]) const {
        ...
        if (SymbolInfoTick(this.marketContext.symbolName, tick)) {
            bid = tick.bid;
            ask = tick.ask;
        } else {
            bid = SymbolInfoDouble(this.marketContext.symbolName, SYMBOL_BID);
            ask = SymbolInfoDouble(this.marketContext.symbolName, SYMBOL_ASK);
        }
        ...
        double dailyHigh = iHigh(this.marketContext.symbolName, PERIOD_D1, 0);
        double dailyLow = iLow(this.marketContext.symbolName, PERIOD_D1, 0);
        ...
        int index = 0;
        commonValues[index++] = this.marketContext.symbolName;
