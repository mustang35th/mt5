...
public:
    ...
    bool update(
        ...
        if (!this.ensureInitialized(this.symbolName, this.timeFrame)) {
        ...
        if (!this.getCloseValue(this.symbolName, this.timeFrame, 1, closeValue)) {
    ...
    bool getClosePosition(string fromSymbolName, ENUM_TIMEFRAMES fromTimeFrame, ENUM_EMA200_POSITION &position) {
        ...
        if (!this.getEmaValue(fromSymbolName, fromTimeFrame, 1, emaValue)) {
            return false;
        }

        if (!this.getCloseValue(fromSymbolName, fromTimeFrame, 1, closeValue)) {
            return false;
        }
    ...
    bool getSlopePips(
        ...
        if (!this.getEmaValue(fromSymbolName, fromTimeFrame, 1, emaShift1Value)) {
            return false;
        }

        if (!this.getEmaValue(fromSymbolName, fromTimeFrame, fromCompareBarIndex, emaCompareValue)) {
            return false;
        }
    ...
    bool getSlopeDirection(
        ...
        if (!this.getSlopePips(fromSymbolName, fromTimeFrame, fromCompareBarIndex, value)) {
            return false;
        }
...
private:
    ...
    bool ensureInitialized(string fromSymbolName, ENUM_TIMEFRAMES fromTimeFrame) {
        if (this.ema200HandlePool != NULL) {
            ...
            this.ema200HandlePool.setTimeframesFromMn1To(fromSymbolName, fromTimeFrame);

            int poolHandle = this.ema200HandlePool.getEma200Handle(fromTimeFrame);
        ...
        if (!this.isInitialized) {
            ...
            if (this.timeFrame != fromTimeFrame) {
                needRecreate = true;
            }
        ...
        this.releaseHandle();
        this.ema200Handle = iMA(fromSymbolName, fromTimeFrame, 200, 0, MODE_EMA, PRICE_CLOSE);
    ...
    bool copyEmaValues(int maxShift, double &emaBuffer[]) {
        ...
        int bars = Bars(this.symbolName, this.timeFrame);
    ...
    bool getCloseValue(
        ...
        int copied = CopyClose(fromSymbolName, fromTimeFrame, fromShiftValue, 1, closeBuffer);
    ...
    void setTextLabels() {
        ...
        string labelTimeFrame = this.timeFrameLabel;

        if (labelTimeFrame == "") {
            labelTimeFrame = TimeUtil::convertTimeFrameToString(this.timeFrame);
        }
