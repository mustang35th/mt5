//+------------------------------------------------------------------+
//|              ZigZagElliotAlertRunExecutionProgressMigration.mqh |
...
#ifndef MSTNG_ZZE_RUN_EXECUTION_PROGRESS_MIGRATION_MQH
#define MSTNG_ZZE_RUN_EXECUTION_PROGRESS_MIGRATION_MQH

#include <Mstng\Log\Logger.mqh>

/**
 * 既存Runテーブルへ実行進捗列を非破壊で追加するクラス。
 *
 * 既存行はLEGACYとして保持し、実行進捗を推測して補完しない。
 */
class ZigZagElliotAlertRunExecutionProgressMigration {
public:
    /**
     * Run実行状態と進捗列の存在を保証する。
     *
    ...
    static bool execute(const int fromDatabaseHandle) {
        ...
        if (!executeSql(
                ...
                "execution progress migration write lock",
                logger
            )) {
...
private:
    ...
    static bool ensureColumn(
        ...
        if (DatabaseExecute(fromDatabaseHandle, sql)) {
            fromLogger.info(
                __FUNCTION__,
                "Run execution progress column added. column="
                    + fromColumnName
            );
...
#endif // MSTNG_ZZE_RUN_EXECUTION_PROGRESS_MIGRATION_MQH
