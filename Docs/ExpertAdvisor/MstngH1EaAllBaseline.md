...
開始日`2026/07/27`、終了日`2026/07/28`、売買開始日時`2026.08.01 00:00:00`とすると、売買開始前の履歴準備だけを確認できます。GBPAUD/H1、リアルティック、最適化なし、ビジュアルON・パネルON、基準CSVはfalseにします。1日で履歴が揃う保証はなく、短い準備期間で足りるかを確認するための試行です。

v1.07ではテスターのログで`PREPARATION_HISTORY`を検索します。以下は形式を示す架空の例です（実測値ではありません）。実際は1行にMN1～H1の全5時間足が並びます。

```text
PREPARATION_HISTORY symbol=GBPAUD simulatedTime=2026.07.27 00:00:30 H1=… state=WAIT_HISTORY reason=ANALYSIS_HISTORY_UNAVAILABLE MN1[WAIT,sync=1,bars=19,required=61,first=2025.01.01 00:00], W1[WAIT,sync=1,bars=82,required=206,first=2025.01.05 00:00], …
```

`bars`が取得本数、`required`が必要本数、`sync=1`が同期済み、`first`がテスターから参照できる最古のバーです。28通貨すべての`state=READY`で履歴準備条件を通過したことを確認します。画面の状態は売買開始前なので「開始前」のままで、稼働28になることを合格条件にしません。READYでも波動分析と指標計算は別途確認が必要です。

初回と準備状態の切り替わりは即時、待機中の内容変化は最短1時間、同一内容は1日間隔で記録します。いずれもテスト内時刻です。最初からREADYの場合も本数を記録し、READY継続は再出力しません。ログは通貨別Runの`Common/Files/MstngH1Ea/Logs/<runUid>.log`にも残ります。CSV出力をfalseにしても診断ログは有効です。
