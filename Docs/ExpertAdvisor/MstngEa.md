...
- 現在のシンボルと時間足
- H1・M5の`MTF_3in3`ではMN1～現在足、それ以外では従来どおりD1以下のOscillatorハンドルプール
- シグナル回数管理
- EA設定とMagic Number
- 戦略Adapter
- ポジション取得・発注サービス
...
1. 現在時間足までの`ElliotAll`を分析する。H1・M5の`MTF_3in3`ではMN1から開始する。H1では本解析のW1を方向判定とW1診断で共用する
2. 任意の通貨強弱実行情報を読み込む
...
構造ランクはH1・H4の同じ波動条件を使用します。短い3波の後の第5波は正常構造として評価し、H1または上位足が第5波の場合は`LATE`を付けます。構造表示そのものは、引き続きエントリー可否を直接制限しません。
...
M5はv1.10から、H1と共通の`Mtf3In3HigherTimeFrameDecision`による上位足フィルタを必須とします。M5方向を基準に、W1・D1の分析方向一致、MN1分析方向またはW1 EMA200の同方向一致、D1 EMA200の同方向一致が必要です。M5固有の分析方向条件等とANDで適用し、Signal Count加算前に判定します。上位足の波動番号や確定状態は問いません。

M5はv1.14から、分析方向をW1＝D1＝M15＝M5、かつH4またはH1がM5と同方向とします。H4・H1が両方同方向でも許可し、両方逆方向なら拒否します。両足の分析データは必要です。EMA200は引き続きD1・H4・H1・M15・M5の5足すべてがM5方向と一致することを要求します。

M5・MTF_3in3もMN1からハンドルと本解析を準備します。上位足の履歴不足・分析失敗ではエントリーしません。分析開始足の変更により波動やZigZag由来SLが変わる場合があります。初回処理・再試行・注文・決済の順序は維持します。M15と他戦略は対象外です。

| 項目 | M15 | M5 |
|---|---|---|
| 対象波 | H4、H1、M15が`1`または`3` | M5主波が`1`・`3`・`A`・`C`（H1・M15の波動番号は制限なし） |
| ZigZag確定 | 現在足の確定を要求 | 現在足の確定を要求 |
| EMA200 | 上位2足・上位1足・現在足の共通条件 | D1・H4・H1・M15・M5がすべてM5方向と一致（v1.11以降） |
| EMA200距離 | 25 pips以内 | v1.12から制限なし（診断のみ） |
| 追加条件 | なし | M5第3波・C波のFEが161.8%以下 |
| 表示波重複制限 | なし | input有効時、同一H1表示波へ1回 |
| `isSendMail`判定値 | `false` | `true`（EA自身は実送信しない） |

M5はv1.15から、元H4・H1の片方だけがM5と逆方向の場合、その足の方向を補正した別の`ElliotAll`をMN1からM5まで生成してから`isJudge()`を実行します。両方一致は元の分析を使用し、両方逆方向は拒否します。`selectJudgmentElliotAll()`で判定用分析を1回選択して現在足・上位足・最新点などを一括設定し、分析方向条件、EMA200・GMMA・Spread・通貨強弱、Wave方向・通常点・主波・FEを同じ採用分析で判定します。指標値は元の観測値をコピーしています。新規エントリーの損切りも同じ採用分析の`lossCut.lc5`を使用します。Signal Countの識別キー、H1表示波の重複キー、分析CSVと通常版ZigZagElliotの既存DB分析列は別に保持した元分析を使用します。決済判定は補正を選択せず元分析を使用します。上位足の許可波動番号は追加しません。

補正では元のOHLC・Oscillatorを保持し、方向変更をH4またはH1だけに限定して、下位足をM5まで再分析します。全7足の直近2本のOHLC・バー時刻を再分析前後に照合し、変更・取得不能・再分析失敗時はエントリーを見送ります。過去履歴全体の固定再計算ではありません。元のM5波動で先に候補を除外せず、最終判定通過後のCount加算→FE判定という既存順序を維持します。通常版ZigZagElliotのAlert DBには補正前後を追加保存しますが、MstngEaの保存経路には追加しません。既存のAlert検証CSVなどの分析値・損切り欄は元分析のままなので、補正採用時の新規注文SLとは異なる場合があります。

通常版`ZigZagElliot`のM5チャートアラート文字は採用分析を使用し、補正時に`[H1補正]`または`[H4補正]`を付けます。MstngEaは内部戦略のチャート描画を実行しないため、この変更はEA自身のシグナル表示には適用しません。EAが使用する`alertText`と既存分析出力は元分析を維持します。通常版M5のメールは、補正時にチャート文字と同じアラート件名を使用し、共通情報・方向変更概要に続けて補正後全体（判定・損切りに採用）と補正前全体（比較用）を記載します。両方とも損切りと全7足の同じ項目を含み、実送信とメール検証ファイルで件名・本文を共有します。補正なしは従来の1セットです。MstngEa自身は引き続きメールを送信しません。

M5はv1.13から最新主波ラベルだけで1・3・A・C波を許可します。副次波ラベルは使用せず、`C.iii`はC波として判定します。3波・C波はFEが有効な正数で、小数1桁へ丸めて161.8%以下であることが必要です。未取得・無効値・0以下は拒否し、上限ちょうどは許可します。1波・A波はFE制限なしです。入口と最終Entry判定に同じM5専用波動判定を使用します。FE拒否コードは3波が`M5_ELLIOT3_FE_REJECTED`、C波が`M5_ELLIOTC_FE_REJECTED`です。

H1はEMA200距離の50 pips比較結果を診断用にのみ保存します。M15は25 pips以内をエントリー条件に使用します。M5はv1.12から25 pipsの距離制限を一旦無効にし、距離と上限比較結果の計算・記録のみ維持します。EMA200方向の5足一致は引き続き必須です。

M15の共通EMA200条件では、上位2足はエントリー方向または`NONE`、上位1足と現在足はエントリー方向を要求します。M5はv1.11からH4の同方向一致を追加し、H1のNONE許容を廃止します。5足のいずれかがNONE・逆方向・取得不能・フラグ／ラベル／時間足不整合なら拒否します。
...
初期SLは、現在足の1つ前のZigZag基準点から損失側へ5 pipsの`lossCut.lc5`です。M5で補正分析を採用した場合は補正後の値を使用し、補正不要の場合は元分析の値を使用します。
...
補正後SLの適用は新規注文が対象です。保有中ポジションのSL更新・トレーリング・決済ロジックは変更しません。新規ポジションの初期リスクと利益戻し状態は、実際のポジションSLから従来どおり取得します。取引結果CSV・決済結果CSVに独立したSL列はありません。
...
### 14.5 通常版ZigZagElliotのAlert DBとの関係

通常版ZigZagElliotのAlert DBはschemaVersion 7で、元分析を既存領域へ保持し、補正採用時の全7足・各足の最新Waveポイント・補正メタデータを同じ`alert_id`へ一括保存します。新規記録は`NONE`または`APPLIED`、補正情報のない旧行は`UNRECORDED`です。重複保存では最初の全体を保持します。元と採用後のSLは分析時点の候補であり、brokerの実SLとは区別します。既存親Alertの判定結果列は採用分析、既存アラート文字・SL・時間足・ポイントは元分析という意味を維持します。

追加先は`zigzag_elliot_alert_corrections`、`zigzag_elliot_alert_corrected_timeframes`、`zigzag_elliot_alert_corrected_points`です。元の`signalReferencePointTime`を比較キーとして維持し、補正基準点は`correctedReferencePointTime`へ分離します。採用後のアラート文字は追加メタデータ、元の文言は親Alertの`alert_text`へ保持します。元・補正後の`getText()`全文も保存します。保存対象は`isAlert`成立時であり、`isJudge()`拒否は対象外です。

この追加はMstngEaの取引CSV・決済CSV・Alert検証CSV・利益戻し状態ファイルを変更しません。H1/M5観測DBも対象外です。Viewerの補正前後比較表示は未対応です。詳細は[ZigZagElliot仕様書](../Indicator/ZigZagElliot.md)を参照してください。
