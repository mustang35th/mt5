...
M5はv1.10から、H1と共通の`Mtf3In3HigherTimeFrameDecision`による上位足フィルタを必須とします。M5方向を基準に、W1・D1の分析方向一致、MN1分析方向またはW1 EMA200の同方向一致、D1 EMA200の同方向一致が必要です。既存のD1～M5方向一致等とANDで適用し、Signal Count加算前に判定します。上位足の波動番号や確定状態は問いません。
...
H1はEMA200距離の50 pips比較結果を診断用にのみ保存します。M15・M5は従来どおり25 pips以内をエントリー条件に使用します。
