...
EA 1.02の高速ウォームアップ追加後の確認結果は次のとおりです。
...
- MT5でのMQL5実行、期間跨ぎ・約定を伴うStrategy Tester確認、速度の実測は未実施。コンパイルと静的/SQLiteテストの成功を、実行時の安全性や処理速度の実証とは扱わない。
