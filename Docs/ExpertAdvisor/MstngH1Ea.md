...
エントリー互換性の基準は、2026-09-06時点の通常版`ZigZagElliot` v1.33をH1チャートで動かした場合の`MTF_3in3`初期設定です。条件だけでなく、分析開始時間足、判定周期およびJudge成立回数の扱いを合わせます。実際の発注には、既定のSL・保有数・DBなどの安全条件を別途適用します。

EA v1.08では、方向一致・W1追加診断・EMA200確認・分析開始足を[Mtf3In3H1Policy](../../Include/Mstng/ExpertAdvisor/Mtf3In3H1Policy.mqh)へ集約し、通常版v1.37・List v1.34のH1 Alert・MstngEa v1.09と同じ設定を参照します。先行診断・本判定・canonical設定記録も同じ参照元に揃えます。MstngH1Eaの実効条件、canonical文字列の形式・順序とconfig hash、戦略V2、DBスキーマ、評価周期、Judge成立回数の消費順序は変更しません。Listの表示条件や各EAの発注安全条件は共通化の対象外です。
...
## 通貨別処理とTimer管理の分離（v1.09・2026-09-23）

複数通貨化の第1段階として、`H1EaEventTimer`へ1秒／30秒のTimer設定、設定成功状態、失敗後5秒の再試行を分離しました。`H1EaController`の既存`startTimer()`・`onTick()`・`onTimer()`は単一通貨版のイベント入口として維持します。Timer終了は従来どおりEAの`OnDeinit()`が担当します。

通貨別Controllerに、Timerを操作しない以下の入口を追加しました。

| メソッド | 責務 |
|---|---|
| `processMaintenance()` | DB保存・再接続・Lease更新。高速保守要求時も安全条件を確認 |
| `processTradeReconciliation()` | 管理権限の更新とbroker取引状態の照合 |
| `processProtection(barTime)` | 取引照合とpending SL・候補跨ぎ処理。波動分析なし |
| `processTrail(barTime)` | H1新規バーのトレイル分析と候補反映 |
| `processEntry(normalTimerReady)` | 呼出元の通常Timer準備状態を受け取り、既存のEntry処理を実行 |
| `processWarmup()` | Tester売買開始前の履歴準備 |
| `canUseFastTesterWarmup()` | 通貨単位で高速ウォームアップ可能か確認 |

単一通貨版はTickで保守→保護→トレイル→Tester Entry、Timerで保守→照合→LIVE Entryを実行します。LIVEの初回1秒・以降30秒、Tester分析失敗時の1秒再試行、H1ごとの初回成功判定、Judge回数の消費順序を維持します。Entry本体はTimer準備状態の受け取り方以外を変更していません。

Timerを通常周期へ戻す試行では、失敗時も高速期間の保守待ちを解除します。設定失敗時はEntryを止め、Tick側の保護処理は継続します。外部スケジュール側は、実際のTimer設定成功状態を`processEntry()`へ渡し、保守・保護・トレイル・Entryの順序と通貨自身のH1バー時刻を管理する必要があります。

今回の段階では28通貨の登録・全体巡回・全体ポジション上限を追加しません。DBスキーマ、Magic、復元キー、設定Canonical Text、戦略バージョンは維持します。

検証：EAのMetaEditorコンパイルはエラー0・警告0。関連Python静的契約テスト85件成功。変更前後のイベントメソッドをC#へ構文変換し、Timer・時刻・分析・取引・DBなどの外部処理をfixtureへ差し替えた6,144回の処理順序・状態比較が一致しました。これはMT5での実売買・Strategy Tester実行の検証ではありません。EAのビルドは一時ディレクトリで行い、稼働先のex5は更新していません。


## 複数通貨DB対応の共通更新（v1.10）

単一通貨版も物理DB schema v3（Runへのnullableな`session_uid`追加）へ対応しました。単一版のsession_uidはNULL、LIVE contextKey・Magic・SignalCount復元・戦略・設定canonicalは従来どおりです。取引状態のDB読取を`H1EaTradeExecutor.restoreFromDatabase()`へ切り出し、通常版では引き続きその後にbroker照合を行います。詳細は[複数通貨版 第3段階](MstngH1EaAll.md#13-第3段階の実装2026-09-23)および[DB schema v3](../Database/MstngH1EaDatabase.md#16-複数通貨runと物理schema-v32026-09-23)を参照してください。


## 複数通貨の保護接続に伴う共通更新（v1.11）

取引通知の軽量受付を`H1EaTradeExecutor.observeTradeTransaction()`へ分離しました。単一通貨版の通知入口は受付直後に従来どおり照合し、Timer・Tick・Entry・トレイルの周期と戦略V2は維持します。複数通貨版だけは照合を後続イベントへ委ねます。気配の時間検証も複数通貨の保護有効化時だけ追加します。DBは物理v3、単一版の設定canonicalは従来どおりです。[複数通貨版 第4段階](MstngH1EaAll.md#14-第4段階の実装2026-09-23)を参照してください。

## 複数通貨Entry接続に伴う共通更新（v1.12）

全通貨版v1.03から共通の`evaluateEntry()`を呼び出す専用巡回入口を追加しました。単一通貨版のTimer/Tick周期、Judge・SignalCount・発注前安全判定、戦略V2、設定canonicalは維持します。分析後と送信直前の追加気配確認は外部巡回モードだけへ適用します。詳細は[複数通貨版 第5段階](MstngH1EaAll.md#15-第5段階の実装2026-09-23)を参照してください。

## 複数通貨Tester準備に伴う共通更新（v1.13）

全通貨版v1.04向けに、子Controllerの高速準備可否確認と外部巡回用の30秒保守、通常復帰時の保守待ち解除を追加しました。単一通貨版の高速化条件・Tick/Timer処理・戦略・設定canonicalは維持します。共通Timerには終了後の状態リセット用メソッドを追加し、全通貨版の親から使用します。詳細は[複数通貨版 第6段階](MstngH1EaAll.md#16-第6段階の実装2026-09-23)を参照してください。

## 複数通貨の状態表示・計測に伴う共通更新（v1.14）

全通貨版v1.05向けに、Controllerの状態コピーと外部巡回時の分析時間計測、EntryState・Timerの表示用getterを追加しました。単一版の戦略・SignalCount・周期・設定canonicalは維持し、パネルinputも追加しません。詳細は[複数通貨版 第7段階](MstngH1EaAll.md#17-第7段階の実装2026-09-23)を参照してください。
