...
第1段階では暗黙のSQL NULLを使用しません。任意情報は利用可能フラグと0または空文字で表現します。これにより、未取得と有効な0をフラグで区別します。後方互換migrationで追加するH1 Observationの`spread_pips`と`pip_size`だけは、既存テーブルへ非破壊で追加できるよう物理列をnullableとします。`spread_pips`の過去行は推測で補完せず、`NULL`を未記録、新規行の`0.0`を有効値として区別します。
...
現行のAlert Runは`schema_version = 5`、`program_version = 1.29`、`strategy_version = MTF3IN3_V5`です。現行のH1 Observation Runは`schema_version = 4`、`program_version = 1.02`、`strategy_version = H1_OBSERVATION_ALL_V3`です。既存Runのバージョンは書き換えません。
...
Alertの`snapshot_hash`はアラート判定、H1方向一致診断、時間足別分析および保存対象となる最新Wave全ポイントから生成します。H1 Observationの`snapshot_hash`は、取得時の`spread_pips`と`pip_size`を含む保存対象の観測結果から生成します。`pip_size`追加後のHash payloadは`H1_OBSERVATION_V3`です。既存行のHashは再計算しません。いずれも分析設定を識別する`analysis_input_hash`とは用途が異なります。
