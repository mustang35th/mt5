...
対象通貨は`USD`、`JPY`、`EUR`、`GBP`、`AUD`、`NZD`、`CAD`、`CHF`の8通貨です。28通貨ペアをD1、H4、H1、M15の4時間足で判定します。
...
| `currency_strength_pair_votes` | 112（28ペア × 4時間足） |
| `currency_strength_results` | 8（通貨数） |
| `currency_strength_contributions` | 224（112票 × BASE/QUOTE） |

各通貨は1時間足につき7ペアへ登場するため、完全な集計における時間足別スコアは-7～+7、全4時間足の合計は-28～+28です。
...
| 4 | `calculation_version` | TEXT | string | NOT NULL | 集計ルール識別子。現行は`pair-direction-raw-v1` |
...
| 9 | `valid_pair_count` | INTEGER | int | NOT NULL | 4時間足すべてを取得できた有効通貨ペア数 |
| 10 | `vote_count` | INTEGER | int | NOT NULL | 保存した票数。`valid_pair_count × 4` |
...
| 4 | `time_frame_order` | INTEGER | int | NOT NULL | 0=D1、1=H4、2=H1、3=M15 |
...
| 8 | `time_frame_text` | TEXT | string | NOT NULL | 時間足表示文字列。例: `D1`、`H4`、`H1`、`M15` |
...
| 4 | `d1_score` | INTEGER | int | NOT NULL | D1の未正規化合計 |
| 5 | `h4_score` | INTEGER | int | NOT NULL | H4の未正規化合計 |
| 6 | `h1_score` | INTEGER | int | NOT NULL | H1の未正規化合計 |
| 7 | `m15_score` | INTEGER | int | NOT NULL | M15の未正規化合計 |
| 8 | `total_score` | INTEGER | int | NOT NULL | 4時間足の未正規化合計 |
| 9 | `d1_sample_count` | INTEGER | int | NOT NULL | D1の票数 |
| 10 | `h4_sample_count` | INTEGER | int | NOT NULL | H4の票数 |
| 11 | `h1_sample_count` | INTEGER | int | NOT NULL | H1の票数 |
| 12 | `m15_sample_count` | INTEGER | int | NOT NULL | M15の票数 |
| 13 | `total_sample_count` | INTEGER | int | NOT NULL | 4時間足の合計票数 |
| 14 | `updated_at` | INTEGER | datetime | NOT NULL | スナップショットのDB保存時刻 |
| 15 | `updated_at_text` | TEXT | string | NOT NULL | `updated_at`の目視用文字列 |
...
- 部分保存を有効にした場合も、有効な1通貨ペアは必ず4票です。
- Serviceは保存前に`vote_count == valid_pair_count × 4`を検証します。
...
- 空の`time_frame_text`はD1、H4、H1、M15へ変換し、その他の値は時間足数値の文字列にします。
...
- 自動移行の対象は表示列と更新時刻列だけです。それ以外の列や制約の差異は自動再構築しません。
...
- Run: 1件
- USDJPY BUY票: D1、H4、H1、M15の4件
- Result: USD、JPYの2件
- Contributionsビュー: 8件

Run ID、件数、先頭票の時間足・バー時刻文字列、3テーブルの`updated_at`と`updated_at_text`の一致を検証します。DBファイルと保存したテストレコードは実行後も残ります。
