...
These assertions inspect production MQL control flow and preserved H1 method
fingerprints. A restricted Python AST evaluator checks extracted boolean
...
class ObservationWarmupContractTest(unittest.TestCase):
    ...
    def test_h1_preflight_and_event_paths_unchanged(self):
        golden = {
            "void processTesterPreflight(": "745bbe026d9160f8f74a6520cdb47c9a7fe3ec50ec40b47d0c8dcc9c178b54aa",
    ...
    def test_h1_history_readiness_preserved_after_m5_dispatch(self):
        readiness = body(self.source, "bool isAnalysisSeriesReady(")
        dispatch = re.compile(
            r"\n\s*if \(this\.observationProfile\.isM5\(\)\) \{\s*"
            r"return this\.isM5AnalysisSeriesReady\(fromIndex\);\s*\}\n")
        unchanged, removed = dispatch.subn("", readiness, count=1)
        self.assertEqual(removed, 1)
        self.assertEqual(hashlib.sha256(unchanged.encode()).hexdigest(),
                         "b2548b6bbba28e0bb1f634d21512b83974c0fb933a45b3e2b6d1686a64acc1cc")
    ...
    def test_m5_checks_all_profile_feet_without_first_failure_return(self):
        readiness = body(self.source, "bool isM5AnalysisSeriesReady(")
        self.assertIn("this.observationProfile.getObservationTimeFrameCount()", readiness)
        self.assertIn("this.observationProfile.getObservationTimeFrame(i)", readiness)
        self.assertIn("requiredBars = 206", readiness)
        self.assertIn("requiredBars = 61", readiness)
        self.assertIn("WarmUpSeriesUtil::isSeriesSynchronized(", readiness)
        self.assertIn("Bars(", readiness)
        self.assertIn("SERIES_FIRSTDATE", readiness)
        loop = body(readiness, "for (int i = 0;")
        self.assertNotIn("return false", loop)
        self.assertNotIn("return true", loop)
        self.assertIn("this.logM5HistoryStatus(fromIndex, historyText)", readiness)

    def test_sufficient_feet_are_excluded_from_history_change_text(self):
        readiness = body(self.source, "bool isM5AnalysisSeriesReady(")
        sufficient = condition(readiness, "if (isSynchronized && availableBars >= requiredBars)")
        for synced, bars, required, expected in ((True, 61, 61, True), (True, 60, 61, False),
            ...
            with self.subTest(synced=synced, bars=bars, required=required):
                self.assertEqual(evaluate_predicate(sufficient, isSynchronized=synced,
                                                    availableBars=bars, requiredBars=required), expected)
        self.assertEqual(body(readiness, "if (isSynchronized && availableBars >= requiredBars)").strip(), "continue;")
        self.assertLess(readiness.index("continue;"), readiness.index("SERIES_FIRSTDATE"))
        for field in ("bars=%d", "required=%d", "first=%s", "synced=%d"):
            self.assertIn(field, readiness)
        self.assertIn('firstDateText = "unavailable"', readiness)
        self.assertIn('return historyText == "";', readiness)
    ...
    def test_missing_current_m5_still_reports_specific_history_shortage(self):
        ...
        branch = body(missing, "if (this.observationProfile.isM5())")
        self.assertIn("this.isM5AnalysisSeriesReady(fromIndex)", branch)
        self.assertLess(missing.index("this.isM5AnalysisSeriesReady(fromIndex)"),
                        missing.index("this.warmUpSymbol(fromIndex)"))
