//+------------------------------------------------------------------+
//|       ZigZagElliotH1StudyConditionStabilityExporter.mq5 |
...
/** 条件安定性集計CSVのスキーマバージョン。 */
const string conditionCsvSchemaVersion =
    "H1_STUDY_CONDITION_STABILITY_V1";
...
/** 時期バケット判定のバージョン。 */
const string conditionPeriodBucketPolicy = "EPISODE_START_JST_V1";
...
/** 条件安定性集計CSVの列数。 */
const int conditionCsvFieldCount = 58;

/** 1分類バケットあたりの集計行数。 */
const int conditionRowsPerDimension = 216;

/** 既定R1の分類バケット数。 */
const int conditionReferenceDimensionCount = 39;

/** 既定R1の条件安定性集計CSV期待行数。 */
const int conditionReferenceRowCount = 8424;
...
/** 条件安定性の集計分類。 */
struct ZigZagElliotH1StudyConditionStabilityDimension {
    /** 分類種別。 */
 ...
 * 分類バケットを含む集計行位置を取得する。
 *
 * @param fromDimensionIndex 分類バケット位置。
 * @param fromRuleIndex 条件配置位置。
 * @param fromConfirmationIndex 確認本数配置位置。
 * @param fromHorizonIndex 評価期間配置位置。
 * @param fromSideScopeIndex 方向集計配置位置。
 * @return 集計行配置位置。
 */
int getConditionRowIndex(
    const int fromDimensionIndex,
    const int fromRuleIndex,
    const int fromConfirmationIndex,
    const int fromHorizonIndex,
    const int fromSideScopeIndex
) {
    return fromDimensionIndex * conditionRowsPerDimension
        + getConditionRowIndex(
            fromRuleIndex,
...
/**
 * 分類種別と分類値に一致する配置位置を取得する。
 *
 * @param fromDimensions 分類一覧。
 * @param fromDimensionType 分類種別。
 * @param fromDimensionValue 分類値。
 * @return 一致位置。存在しない場合は-1。
 */
int findConditionDimensionIndex(
    const ZigZagElliotH1StudyConditionStabilityDimension &fromDimensions[],
    const string fromDimensionType,
    const string fromDimensionValue
) {
    int dimensionCount = ArraySize(fromDimensions);

    for (int i = 0; i < dimensionCount; i++) {
        if (fromDimensions[i].dimensionType == fromDimensionType
                && fromDimensions[i].dimensionValue
                    == fromDimensionValue) {
            return i;
        }
    }
 ...
 * @return 追加できた場合true。
 */
bool addConditionDimension(
    ZigZagElliotH1StudyConditionStabilityDimension &fromDimensions[],
    const string fromDimensionType,
    const string fromDimensionValue,
    Logger &fromLogger
) {
    if (fromDimensionType == "" || fromDimensionValue == "") {
        fromLogger.error(__FUNCTION__, "Dimension value is empty.");

        return false;
    }

    if (findConditionDimensionIndex(
            fromDimensions,
            fromDimensionType,
            fromDimensionValue
        ) >= 0) {
        fromLogger.error(
            __FUNCTION__,
            "Duplicate dimension. type=" + fromDimensionType
                + " value=" + fromDimensionValue
        );

        return false;
    }

    int dimensionIndex = ArraySize(fromDimensions);

    if (ArrayResize(fromDimensions, dimensionIndex + 1, 64)
            != dimensionIndex + 1) {
        fromLogger.error(__FUNCTION__, "Dimension array allocation failed.");

        return false;
    }

    fromDimensions[dimensionIndex].reset();
    fromDimensions[dimensionIndex].dimensionType = fromDimensionType;
    fromDimensions[dimensionIndex].dimensionValue = fromDimensionValue;

    return true;
}

/**
 * 研究期間とStreamから決定的な順序の分類一覧を構築する。
 *
 * @param fromStreams 参照元Stream一覧。
 * @param fromDimensions 分類一覧。
 * @param fromLogger ロガー。
 * @return 分類一覧を構築できた場合true。
 */
bool initializeConditionDimensions(
    const ZigZagElliotH1StudyStreamKey &fromStreams[],
    ZigZagElliotH1StudyConditionStabilityDimension &fromDimensions[],
    Logger &fromLogger
) {
    ArrayResize(fromDimensions, 0);
    MqlDateTime fromDateParts;
    MqlDateTime toDateParts;

    if (!TimeToStruct(studyFromJstTime, fromDateParts)
            || !TimeToStruct(studyToJstTime - 1, toDateParts)) {
        fromLogger.error(__FUNCTION__, "Study range conversion failed.");

        return false;
    }

    if (!addConditionDimension(
            fromDimensions,
            "OVERALL",
            "ALL",
            fromLogger
    ...
    for (int i = fromDateParts.year; i <= toDateParts.year; i++) {
        if (!addConditionDimension(
                fromDimensions,
                "YEAR",
                IntegerToString(i),
    ...
    int streamCount = ArraySize(fromStreams);

    for (int i = 0; i < streamCount; i++) {
        if (findConditionDimensionIndex(
                fromDimensions,
                "SYMBOL",
                fromStreams[i].symbolName
            ) >= 0) {
            continue;
        }
...
/**
 * Episode開始JSTに対応するYEAR分類位置を取得する。
 *
 * @param fromEpisodeStartJstTime Episode開始JST。
 * @param fromDimensions 分類一覧。
 * @return YEAR分類位置。取得できない場合は-1。
 */
int findConditionYearDimensionIndex(
    const datetime fromEpisodeStartJstTime,
    const ZigZagElliotH1StudyConditionStabilityDimension &fromDimensions[]
) {
    MqlDateTime dateParts;

    if (!TimeToStruct(fromEpisodeStartJstTime, dateParts)) {
        return -1;
    }

    return findConditionDimensionIndex(
        fromDimensions,
        "YEAR",
        IntegerToString(dateParts.year)
    );
}

/**
 * Episode開始JSTに対応するQUARTER分類位置を取得する。
 *
 * @param fromEpisodeStartJstTime Episode開始JST。
 * @param fromDimensions 分類一覧。
 * @return QUARTER分類位置。取得できない場合は-1。
 */
int findConditionQuarterDimensionIndex(
    const datetime fromEpisodeStartJstTime,
    const ZigZagElliotH1StudyConditionStabilityDimension &fromDimensions[]
) {
    MqlDateTime dateParts;
    ...
    int quarter = (dateParts.mon - 1) / 3 + 1;

    return findConditionDimensionIndex(
        fromDimensions,
        "QUARTER",
        StringFormat("%d-Q%d", dateParts.year, quarter)
    );
}
...
bool validateConditionSourceRun(
    ...
    if (fromRunInfo.runId <= 0
        ...
        fromLogger.error(
            __FUNCTION__,
            "Source Run metadata required by condition stability is incomplete."
        );
...
bool initializeConditionRows(
    ...
    int dimensionCount = ArraySize(fromDimensions);
    int expectedRowCount = dimensionCount * conditionRowsPerDimension;

    if (dimensionCount <= 0
            || ArrayResize(fromRows, expectedRowCount) != expectedRowCount
            || ArrayResize(
            ...
            for (int j = 0; j < conditionConfirmationCount; j++) {
                for (int k = 0; k < conditionHorizonCount; k++) {
                    for (int sideIndex = 0;
                        ...
                        fromRows[rowIndex].dimensionType =
                            fromDimensions[dimensionIndex].dimensionType;
                        fromRows[rowIndex].dimensionValue =
                            fromDimensions[dimensionIndex].dimensionValue;
                        ...
                        if (CheckPointer(conditionMedianSamples[rowIndex])
                                != POINTER_DYNAMIC
                                || !conditionMedianSamples[rowIndex].Step(256)) {
                            fromLogger.error(
                                __FUNCTION__,
                                StringFormat(
 ...
 * @param fromSide BUYまたはSELL。
 * @param fromOverallDimensionIndex OVERALL分類位置。
 * @param fromYearDimensionIndex YEAR分類位置。
 * @param fromQuarterDimensionIndex QUARTER分類位置。
 * @param fromSymbolDimensionIndex SYMBOL分類位置。
...
bool aggregateConditionCandidate(
    ...
    const string fromSide,
    const int fromOverallDimensionIndex,
    const int fromYearDimensionIndex,
    const int fromQuarterDimensionIndex,
    const int fromSymbolDimensionIndex,
    ...
    for (int i = 0; i < conditionHorizonCount; i++) {
        ...
        for (int dimensionSlot = 0; dimensionSlot < 4; dimensionSlot++) {
            int dimensionIndex = fromOverallDimensionIndex;

            if (dimensionSlot == 1) {
                dimensionIndex = fromYearDimensionIndex;
            } else if (dimensionSlot == 2) {
                dimensionIndex = fromQuarterDimensionIndex;
            } else if (dimensionSlot == 3) {
                dimensionIndex = fromSymbolDimensionIndex;
            }
...
bool processConditionStream(
    ...
    int overallDimensionIndex = findConditionDimensionIndex(
        fromDimensions,
        "OVERALL",
        "ALL"
    );
    int symbolDimensionIndex = findConditionDimensionIndex(
        fromDimensions,
        "SYMBOL",
        fromStream.symbolName
    );

    if (overallDimensionIndex < 0 || symbolDimensionIndex < 0) {
        fromLogger.error(
            __FUNCTION__,
            "Stream dimension was not found. symbol=" + fromStream.symbolName
        );
    ...
    for (int i = 0; i < conditionRuleCount; i++) {
        ...
        while (rowIndex < rowCount) {
            ...
            if (episodeStartJstTime >= studyFromJstTime
                ...
                int yearDimensionIndex = findConditionYearDimensionIndex(
                    episodeStartJstTime,
                    fromDimensions
                );
                int quarterDimensionIndex =
                    findConditionQuarterDimensionIndex(
                        episodeStartJstTime,
                        fromDimensions
                    );

                if (yearDimensionIndex < 0 || quarterDimensionIndex < 0) {
                    fromLogger.error(
                        __FUNCTION__,
                        StringFormat(
                            "Period dimension was not found. time=%s",
                            TimeToString(
                                episodeStartJstTime,
                ...
                for (int j = 0; j < confirmationLimit; j++) {
                    ...
                    if (!aggregateConditionCandidate(
                            ...
                            side,
                            overallDimensionIndex,
                            yearDimensionIndex,
                            quarterDimensionIndex,
                            symbolDimensionIndex,
                            fromSummaryRows,
                            fromLogger
                        )) {
                        return false;
 ...
 * 加算可能な集計値を合計する。
 *
 * @param fromTarget 合計先。
 * @param fromSource 加算元。
 */
void addConditionRawValues(
    ZigZagElliotH1StudyConditionBreakdownRow &fromTarget,
    ZigZagElliotH1StudyConditionBreakdownRow &fromSource
) {
    fromTarget.candidateEntryCount += fromSource.candidateEntryCount;
    fromTarget.ineligibleEntryCount += fromSource.ineligibleEntryCount;
    fromTarget.eligibleEntryCount += fromSource.eligibleEntryCount;
    fromTarget.eligibleOutcomeCount += fromSource.eligibleOutcomeCount;
    fromTarget.calculatedOutcomeCount += fromSource.calculatedOutcomeCount;
    fromTarget.failedOutcomeCount += fromSource.failedOutcomeCount;
    fromTarget.missingOutcomeCount += fromSource.missingOutcomeCount;
    fromTarget.futureH1GapCount += fromSource.futureH1GapCount;
    fromTarget.otherFailureCount += fromSource.otherFailureCount;
    fromTarget.winningCount += fromSource.winningCount;
    fromTarget.losingCount += fromSource.losingCount;
    fromTarget.breakevenCount += fromSource.breakevenCount;
    fromTarget.netProfitSumPips += fromSource.netProfitSumPips;
    fromTarget.grossProfitSumPips += fromSource.grossProfitSumPips;
    fromTarget.mfeSumPips += fromSource.mfeSumPips;
    fromTarget.maeSumPips += fromSource.maeSumPips;
    fromTarget.grossProfitSumAtr += fromSource.grossProfitSumAtr;
    fromTarget.netProfitSumAtr += fromSource.netProfitSumAtr;
    fromTarget.maxProfitH1BarsSum += fromSource.maxProfitH1BarsSum;
    fromTarget.entrySpreadSumPips += fromSource.entrySpreadSumPips;
    fromTarget.winningNetProfitSumPips +=
        fromSource.winningNetProfitSumPips;
    fromTarget.losingNetProfitAbsSumPips +=
        fromSource.losingNetProfitAbsSumPips;
}

/**
 * 浮動小数の生集計値が再加算誤差の範囲で一致するか判定する。
 *
 * @param fromExpected 期待値。
 * @param fromActual 実値。
 * @return 差の絶対値が1e-6以下の場合true。
 */
bool isConditionRawDoubleEqual(
    const double fromExpected,
    const double fromActual
) {
    return MathAbs(fromExpected - fromActual) <= 0.000001;
}

/**
 * 2行の加算可能な集計値が一致するか判定する。
 *
 * @param fromExpected 期待行。
 * @param fromActual 実行。
 * @return 全値が一致する場合true。
 */
bool isConditionRawValuesEqual(
    ZigZagElliotH1StudyConditionBreakdownRow &fromExpected,
    ZigZagElliotH1StudyConditionBreakdownRow &fromActual
) {
    return fromExpected.candidateEntryCount
            == fromActual.candidateEntryCount
        && fromExpected.ineligibleEntryCount
            == fromActual.ineligibleEntryCount
        && fromExpected.eligibleEntryCount == fromActual.eligibleEntryCount
        && fromExpected.eligibleOutcomeCount
            == fromActual.eligibleOutcomeCount
        && fromExpected.calculatedOutcomeCount
            == fromActual.calculatedOutcomeCount
        && fromExpected.failedOutcomeCount == fromActual.failedOutcomeCount
        && fromExpected.missingOutcomeCount
            == fromActual.missingOutcomeCount
        && fromExpected.futureH1GapCount == fromActual.futureH1GapCount
        && fromExpected.otherFailureCount == fromActual.otherFailureCount
        && fromExpected.winningCount == fromActual.winningCount
        && fromExpected.losingCount == fromActual.losingCount
        && fromExpected.breakevenCount == fromActual.breakevenCount
        && isConditionRawDoubleEqual(
            fromExpected.netProfitSumPips,
            fromActual.netProfitSumPips
        )
        && isConditionRawDoubleEqual(
            fromExpected.grossProfitSumPips,
            fromActual.grossProfitSumPips
        )
        && isConditionRawDoubleEqual(
            fromExpected.mfeSumPips,
            fromActual.mfeSumPips
        )
        && isConditionRawDoubleEqual(
            fromExpected.maeSumPips,
            fromActual.maeSumPips
        )
        && isConditionRawDoubleEqual(
            fromExpected.grossProfitSumAtr,
            fromActual.grossProfitSumAtr
        )
        && isConditionRawDoubleEqual(
            fromExpected.netProfitSumAtr,
            fromActual.netProfitSumAtr
        )
        && isConditionRawDoubleEqual(
            fromExpected.maxProfitH1BarsSum,
            fromActual.maxProfitH1BarsSum
        )
        && isConditionRawDoubleEqual(
            fromExpected.entrySpreadSumPips,
            fromActual.entrySpreadSumPips
        )
        && isConditionRawDoubleEqual(
            fromExpected.winningNetProfitSumPips,
            fromActual.winningNetProfitSumPips
        )
        && isConditionRawDoubleEqual(
            fromExpected.losingNetProfitAbsSumPips,
            fromActual.losingNetProfitAbsSumPips
        );
}

/**
 * 1分類バケット内の件数と方向分解を検証する。
 *
 * @param fromDimensionIndex 分類バケット位置。
 * @param fromRows 集計行配列。
 * @param fromLogger ロガー。
 * @return 全集計行が整合する場合true。
 */
bool validateConditionDimensionRows(
    const int fromDimensionIndex,
    ZigZagElliotH1StudyConditionBreakdownRow &fromRows[],
    Logger &fromLogger
) {
    for (int i = 0; i < conditionRuleCount; i++) {
        for (int j = 0; j < conditionConfirmationCount; j++) {
            int baselineIndex = getConditionRowIndex(
                fromDimensionIndex,
                i,
                j,
                0,
            ...
            for (int k = 0; k < conditionHorizonCount; k++) {
                int allIndex = getConditionRowIndex(
                    fromDimensionIndex,
                    i,
                    j,
                    k,
                ...
                int buyIndex = getConditionRowIndex(
                    fromDimensionIndex,
                    i,
                    j,
                    k,
                ...
                int sellIndex = getConditionRowIndex(
                    fromDimensionIndex,
                    i,
                    j,
                    k,
                ...
                ZigZagElliotH1StudyConditionBreakdownRow sellRow =
                    fromRows[sellIndex];
                ZigZagElliotH1StudyConditionBreakdownRow sideTotalRow;
                sideTotalRow.reset();
                addConditionRawValues(sideTotalRow, buyRow);
                addConditionRawValues(sideTotalRow, sellRow);
                bool isConsistent = allRow.candidateEntryCount
                    ...
                    && allRow.eligibleOutcomeCount
                        == buyRow.eligibleOutcomeCount
                            + sellRow.eligibleOutcomeCount
                    && allRow.calculatedOutcomeCount
                    ...
                    && allRow.missingOutcomeCount
                        == buyRow.missingOutcomeCount
                            + sellRow.missingOutcomeCount
                    && allRow.futureH1GapCount
                        == buyRow.futureH1GapCount
                            + sellRow.futureH1GapCount
                    && allRow.otherFailureCount
                        == buyRow.otherFailureCount
                            + sellRow.otherFailureCount
                    && allRow.winningCount
                    ...
                    && isConditionRawValuesEqual(allRow, sideTotalRow);

                if (!isConsistent) {
                    fromLogger.error(
                        __FUNCTION__,
                        StringFormat(
                            "Condition group count mismatch. dimension=%s/%s rule=%s confirmation=%d horizon=%d",
                            allRow.dimensionType,
                            allRow.dimensionValue,
                            getConditionRuleName(i),
                            j + 1,
                            getConditionHorizonH1Bars(k)
                        )
...
/**
 * 指定分類種別の再合算がOVERALLと一致するか検証する。
 *
 * @param fromDimensionType YEAR、QUARTERまたはSYMBOL。
 * @param fromOverallDimensionIndex OVERALL分類位置。
 * @param fromDimensions 分類一覧。
 * @param fromRows 集計行配列。
 * @param fromLogger ロガー。
 * @return 全集計行が一致する場合true。
 */
bool validateConditionDimensionTotals(
    const string fromDimensionType,
    const int fromOverallDimensionIndex,
    const ZigZagElliotH1StudyConditionStabilityDimension &fromDimensions[],
    ZigZagElliotH1StudyConditionBreakdownRow &fromRows[],
    ...
    int dimensionCount = ArraySize(fromDimensions);
    int matchedDimensionCount = 0;

    for (int rowOffset = 0;
            rowOffset < conditionRowsPerDimension;
            rowOffset++) {
        ZigZagElliotH1StudyConditionBreakdownRow totalRow;
        totalRow.reset();
        ...
        for (int i = 0; i < dimensionCount; i++) {
            ...
            int rowIndex = i * conditionRowsPerDimension + rowOffset;
            addConditionRawValues(totalRow, fromRows[rowIndex]);
            matchedDimensionCount++;
        }

        int overallRowIndex = fromOverallDimensionIndex
            * conditionRowsPerDimension + rowOffset;

        if (matchedDimensionCount <= 0
                || !isConditionRawValuesEqual(
                    fromRows[overallRowIndex],
                    totalRow
                )) {
            fromLogger.error(
                __FUNCTION__,
                StringFormat(
                    "Dimension total mismatch. type=%s row=%d dimensions=%d",
                    fromDimensionType,
                    rowOffset,
                    matchedDimensionCount
                )
...
/**
 * 各YEARに属する4つのQUARTER再合算がYEARと一致するか検証する。
 *
 * @param fromDimensions 分類一覧。
 * @param fromRows 集計行配列。
 * @param fromLogger ロガー。
 * @return 全YEARの全集計行が一致する場合true。
 */
bool validateConditionYearQuarterTotals(
    const ZigZagElliotH1StudyConditionStabilityDimension &fromDimensions[],
    ZigZagElliotH1StudyConditionBreakdownRow &fromRows[],
    Logger &fromLogger
) {
    ...
    for (int yearDimensionIndex = 0;
            yearDimensionIndex < dimensionCount;
            yearDimensionIndex++) {
        if (fromDimensions[yearDimensionIndex].dimensionType != "YEAR") {
            continue;
        }

        string quarterPrefix =
            fromDimensions[yearDimensionIndex].dimensionValue + "-Q";
        ...
        for (int rowOffset = 0;
            ...
            ZigZagElliotH1StudyConditionBreakdownRow totalRow;
            totalRow.reset();
            int matchedQuarterCount = 0;

            for (int i = 0; i < dimensionCount; i++) {
                if (fromDimensions[i].dimensionType != "QUARTER"
                        || StringFind(
                            fromDimensions[i].dimensionValue,
                            quarterPrefix
                        ) != 0) {
            ...
            int yearRowIndex = yearDimensionIndex
                * conditionRowsPerDimension + rowOffset;

            if (matchedQuarterCount != 4
                    || !isConditionRawValuesEqual(
                        fromRows[yearRowIndex],
                        totalRow
                    )) {
                fromLogger.error(
                    __FUNCTION__,
                    StringFormat(
                        "YEAR and QUARTER total mismatch. year=%s row=%d quarters=%d",
                        fromDimensions[yearDimensionIndex].dimensionValue,
                        rowOffset,
                        matchedQuarterCount
                    )
                );
...
bool validateConditionRows(
    ...
    if (dimensionCount <= 0
            || overallDimensionIndex < 0
            || ArraySize(fromRows) != expectedRowCount) {
    ...
    for (int dimensionIndex = 0;
            dimensionIndex < dimensionCount;
            dimensionIndex++) {
        if (!validateConditionDimensionRows(
                dimensionIndex,
                fromRows,
                fromLogger
            )) {
    ...
    if (!validateConditionYearQuarterTotals(
            fromDimensions,
            fromRows,
            fromLogger
        ) || !validateConditionDimensionTotals(
            "YEAR",
            overallDimensionIndex,
            fromDimensions,
            fromRows,
    ...
    if (isConditionReferenceConfiguration(fromRunId)
            && (dimensionCount != conditionReferenceDimensionCount
                || expectedRowCount != conditionReferenceRowCount
                || firstHorizonCandidateCount
        ...
        fromLogger.error(
            __FUNCTION__,
            StringFormat(
                "Reference total mismatch. dimensions=%d rows=%d candidates=%I64d outcomes=%I64d",
                dimensionCount,
                expectedRowCount,
                firstHorizonCandidateCount,
                firstHorizonCandidateCount * conditionHorizonCount
...
string getConditionOutputFileName(const long fromRunId) {
    ...
    return StringFormat(
        "mstng-zigzag-elliot-h1-study-condition-stability-%s-run-%I64d.csv",
...
bool setConditionHeaderValues(
    ...
    fromValues[index++] = "period_bucket_policy";
    fromValues[index++] = "dimension_type";
    fromValues[index++] = "dimension_value";
...
bool setConditionRowValues(
    ...
    fromValues[index++] = conditionPeriodBucketPolicy;
    fromValues[index++] = fromRow.dimensionType;
    fromValues[index++] = fromRow.dimensionValue;
...
/**
 * 時期・通貨別に6累積条件・1/2/3本確認・4期間・方向を集計する。
 */
void OnStart() {
    ...
    ZigZagElliotH1StudyConditionStabilityDimension dimensions[];

    if (!initializeConditionDimensions(streams, dimensions, logger)) {
        rollbackConditionReadTransaction(databaseHandle, logger);
    ...
    if (!initializeConditionRows(dimensions, rows, logger)) {
    ...
    logger.info(
        __FUNCTION__,
        StringFormat(
            "Condition stability aggregation started. sourceRunId=%I64d streams=%d dimensions=%d JST=[%s,%s) rules=%d",
            runInfo.runId,
            streamCount,
    ...
    if (!writeConditionCsvAtomically(runInfo, rows, logger)) {
        logger.error(__FUNCTION__, "Condition stability CSV export failed.");
    ...
    logger.info(
        __FUNCTION__,
        StringFormat(
            "Condition stability export finished. file=%s rows=%d candidates=%I64d outcomes=%I64d",
